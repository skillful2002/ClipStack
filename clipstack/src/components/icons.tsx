// P3 · 内联 SVG 图标（使用 currentColor，便于随文本色着色）。
...
/** 类型图标：依据内容类型展示不同字形。 */
export function TypeIcon({ type, size = 16 }: IconProps & { type: ContentType }) {
  const common = {
    width: size,
    height: size,
    viewBox: "0 0 24 24",
...
/** 全部（汇总）：四宫格，表示所有类型的集合。 */
export function AllIcon({ size = 16 }: IconProps) {
  return (
...
export function PinIcon({ size = 16, active = false }: IconProps & { active?: boolean }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill={active ? "currentColor" : "none"} stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
      <path d="M12 17v5" />
      <path d="M9 10.76V5a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v5.76l2 3.24H7z" />
    </svg>
  );
}

export function StarIcon({ size = 16, active = false }: IconProps & { active?: boolean }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill={active ? "currentColor" : "none"} stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
...
export function TrashBinIcon({ size = 16 }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
      <path d="M3 6h18" />
      <path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2" />
