...
#
# 用法：bash scripts/build-macos-dmg.sh
...
DMG_OUT="${DMG_DIR}/${PRODUCT_NAME}_0.1.0_aarch64.dmg"
STAGE="$(mktemp -d)"
...
echo "==> 构建应用（tauri build）..."
npm run tauri build || echo "注意：tauri build 在 dmg 步骤返回非零（通常是 create-dmg 的 AppleScript 环境问题），继续手动打包 .dmg。"
