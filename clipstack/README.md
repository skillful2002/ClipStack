...
- **一键复制**：点击 / 回车复制回系统剪贴板（文本/链接/代码）；图片/文件复制因平台 API 限制暂未实现，按钮已禁用。
- **托盘 / 菜单栏**：
...
> 自动化测试门禁：Rust `cargo test`（24/24）、`cargo clippy --all-targets`（0 告警）；前端 `npm run build`（tsc + vite）通过。GUI / 剪贴板 / 出包等需在桌面或 CI 手动验证。
