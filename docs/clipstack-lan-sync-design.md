...
> 状态：已实施（v0.2.0-lan）
> 日期：2026-08-10（设计稿） / 2026-08-11（落地完成）
...
- 共享组（文本输入）
- 密钥（密码框，可显隐；留空即保留现有密钥）
- 共享本机剪贴板开关（`share_out`，开启前须先配置共享组 / 密钥 / 端口）
- 手动 peer 列表（IP:端口，跨 VLAN / 组播被禁兜底）
- 组内在线设备列表（来自 mDNS 发现 + 已连 peers，显示设备名）
- 历史条目来源标识（本机 / 设备名）
- 文件大小上限（可配置，默认 100MB）、共享类型过滤（文本 / 图片 / 文件）
- 历史列表悬浮菜单「另存为」：图片存为 PNG、单文件 / 多文件 / 文件夹保存到本地指定位置（经 `tauri-plugin-dialog`）
- 托盘菜单「共享」项：状态圆点（亮/暗）+「已开启·点击关闭 / 已关闭·点击开启」文字，点击切换并实时同步前端
...
| **L3 mesh + 加密** | 多对端广播、`group_fp` 分组、PSK 派生密钥、`share_out`、自动粘贴开关、手动 peer 兜底；32KB 分片传输（大文件 / 目录包） | 多机互拷 + 跨 VLAN 手动配置 | ✅ |
| **L4 UX 与文件** | 局域网共享设置面板、配对参数输入、组内设备列表、来源标识、文件大小上限（可配置）、历史列表「另存为」、托盘「共享」开关（状态圆点） | 手动验收 | ✅ |

### 落地补充（v0.2.0-lan）

- **托盘一键开关**：托盘菜单新增「共享」项，左侧圆点指示状态（亮 = 开、暗 = 关），点击即在开启 / 关闭间切换；切换经 `lan-config-changed` 事件实时同步前端设置页。
- **开启前置校验**：开启共享前必须已设置共享组、共享密钥、端口三者，缺失则拒绝开启并提示（避免半配置状态空跑）。
- **文件分片传输**：大文件与文件夹 / 目录包（如 macOS `.rtfd`）按 32KB 帧切片传输并接收端重组，避免空闲超时误杀；目录包保留层级结构。
