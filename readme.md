...
- **一键复制**：点击或回车即可把条目复制回系统剪贴板（文本 / 链接 / 代码）；图片与文件因平台 API 限制暂未开放复制。
- **菜单栏 / 系统托盘**：macOS 菜单栏、Windows 托盘常驻；下拉展示最近历史，一击即取；条数可在设置中配置（默认 30）。
...
质量门禁：Rust `cargo test`（24/24）、`cargo clippy --all-targets`（0 告警）；前端 `npm run build`（tsc + vite）通过。

---

## 八、已知限制

- 图片 / 文件的一键复制因平台 API 限制暂未实现（对应按钮已禁用），后续版本接入。
- 正式的签名 / 公证 / 出包需在开发者本机或 CI 完成，沙箱环境无法执行。
- macOS 菜单栏托盘图标默认按模板图渲染（可能偏单色），属原生行为。
